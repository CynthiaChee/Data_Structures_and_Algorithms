...
namespace Vector
{
    public class Vector<T> : IEnumerable<T> where T : IComparable<T>
    {
        ...
        public int BinarySearch(T element, IComparer<T> comparer, int left, int right)
        {
            int middle = left + (right - left) / 2;
            if (left > right) return -1;
            if (comparer.Compare(element, data[middle]) == 0)
                return IndexOf(element);
            else if (comparer.Compare(element, data[middle]) > 0)
                return BinarySearch(element, comparer, middle + 1, right);
            else if (comparer.Compare(element, data[middle]) < 0)
                return BinarySearch(element, comparer, left, middle - 1);
            return -1;
        }
        ...
        private class Iterator : IEnumerator<T>
        {
            ...
            object IEnumerator.Current => _data[_currentIdx];
            ...
            {
                _currentIdx++;
                return (_currentIdx < _data.Count);
            }
